...
const names = ["mastercam_doctor", "mastercam_help", "list_tool_categories", "mastercam_plan", "inspect", "measure", "assert", "preview_change", "capture_view", "rollback_change"];
const responses = {}; for (const [index, name] of names.entries()) responses[name] = await call(10 + index, "tools/call", { name, arguments: name === "assert" ? { path: "operation.feed", equals: 35 } : name === "preview_change" ? { feed: 42 } : name === "rollback_change" ? { beforeFeed: 35 } : { operationId: 4 }, _meta: { progressToken: "feature-smoke" } });
...
console.log(JSON.stringify({ initialized: Boolean(init.result), toolCount: list.result.tools.length, progressCount, features: Object.fromEntries(names.map(name => [name, Boolean(responses[name]?.result)])) }, null, 2)); child.kill();
