...
test("invalid operation id never mutates the first operation (BUG-01)", async () => {
  ...
  const result = await backend.call({ id: "t1", tool: "set_feed_speed", arguments: { operationId: 999, feed: 50 } });
...
test("missing operationId with ambiguous selection requires a target (BUG-01)", async () => {
  ...
  const result = await backend.call({ id: "t3", tool: "set_feed_speed", arguments: { feed: 50 } });
  assert.equal(result.ok, false);
  assert.equal(result.error?.code, "TARGET_REQUIRED");
  const single = new MockBackend({ operations: [{ id: 7, name: "Only", feed: 30 }] }, tempAudit());
  const ok = await single.call({ id: "t4", tool: "set_feed_speed", arguments: { feed: 50 } });
...
test("approval tokens are single use and stale fingerprints are refused (BUG-06/07)", async () => {
  ...
  // Mutate the operation behind the server's back: the apply must detect the drift.
  await backend.call({ id: "s5", tool: "set_feed_speed", arguments: { operationId: 4, feed: 77 } });
...
test("rollback refuses when the operation changed after the transaction (BUG-05)", async () => {
  ...
  await backend.call({ id: "r3", tool: "set_feed_speed", arguments: { operationId: 4, feed: 55 } });
  const refused = await backend.call({ id: "r4", tool: "rollback_change", arguments: { transactionId: rollbackToken } });
...
test("audit log records hash-chained entries and verifies (section 30)", async () => {
  ...
  await backend.call({ id: "a1", tool: "set_feed_speed", arguments: { operationId: 4, feed: 50 } });
  await log.flush();
...
test("quantities preserve units and reject invalid values (SAFE-01)", async () => {
  const backend = new MockBackend({ operations: [{ id: 4, name: "Facing", feed: 35 }] }, tempAudit());
  const bad = await backend.call({ id: "q1", tool: "set_feed_speed", arguments: { operationId: 4, feed: -5 } });
  assert.equal(bad.ok, false);
  assert.equal(bad.error?.code, "INVALID_FEED");
  const list = await backend.call({ id: "q2", tool: "list_operations", arguments: {} });
  const op = (list.data as Array<{ feed: { value: number; unit: string } }>)[0];
  assert.equal(op.feed.unit, "mm/min");
  assert.equal(typeof op.feed.value, "number");
});
