...
import { MockBackend, type Fixture } from "../src/backend.js";
import { AuditLog } from "../src/audit/audit-log.js";
import { doctor } from "../src/diagnostics.js";

/**
 * Feature tests exercise fixture behavior, not persistent audit I/O. Give each
 * backend its own disabled audit instance so parallel test workers never race
 * over the process-wide default audit file.
 */
function fixtureBackend(fixture?: Fixture): MockBackend {
...
test("fixture backend supports inspect, measure, assert, preview, and rollback", async () => {
  const backend = fixtureBackend({ feed: 37 });
...
test("fixture feed changes stay scoped to the selected operation", async () => {
  const backend = fixtureBackend({ operations: [{ id: 1, name: "Facing", feed: { value: 30, unit: "mm/min" } }, { id: 2, name: "Pocket", feed: { value: 60, unit: "mm/min" } }] });
...
test("fixture exposes capability discovery, explanation, risks, and machine context", async () => {
  const backend = fixtureBackend({ operations: [{ id: 7, name: "Pocket rough", type: "roughing", feedRate: { value: 60, unit: "mm/min" } }] });
...
test("fixture verifies the reread state after a feed change", async () => {
  const backend = fixtureBackend({ feed: 30 });
...
test("invalid operation ID returns OPERATION_NOT_FOUND", async () => {
  const backend = fixtureBackend({ operations: [{ id: 1, name: "Op1" }, { id: 2, name: "Op2" }] });
...
test("preview requires at least one parameter", async () => {
  const backend = fixtureBackend({ operations: [{ id: 1, name: "Op1", feedRate: { value: 30, unit: "mm/min" } }] });
...
test("stale preview is rejected", async () => {
  const backend = fixtureBackend({ operations: [{ id: 1, name: "Op1", feed: { value: 30, unit: "mm/min" } }] });
