...
test("fixture feed changes stay scoped to the selected operation", async () => {
  const backend = new MockBackend({ operations: [{ id: 1, name: "Facing", feed: { value: 30, unit: "mm/min" } }, { id: 2, name: "Pocket", feed: { value: 60, unit: "mm/min" } }] });
...
test("stale preview is rejected", async () => {
  const backend = new MockBackend({ operations: [{ id: 1, name: "Op1", feed: { value: 30, unit: "mm/min" } }] });
