...
test("every registered tool has a strict input schema and annotations (ARCH-01/03)", () => {
  ...
  const mutations = TOOL_DEFINITIONS.filter(definition => ["apply_operation_parameter_preview", "set_feed_speed", "rollback_change", "regenerate_toolpath"].includes(definition.name));
