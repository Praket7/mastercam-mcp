...
1. **`.TOOLDB` catalog loading:** provide current files for every supported Mastercam release and authoritative schema/API access; verify read-only mappings against tool records known from Mastercam. The current JSON inventory remains operator-maintained.
