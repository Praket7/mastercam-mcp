...
import { DoctorOutputSchema } from "./schemas/diagnostics.js";
...
export async function doctor(pipe: string, backend: string) {
  ...
  const result = { ok: Object.values(checks).every(check => check.ok), tool: "mastercam_doctor", data: { checks, backend, installations, compatibility: compatibilityReport(installations), guidance: "Run the installer as administrator, restart Mastercam, and invoke the Mastercam MCP NET-Hook entry before testing the live pipe." } };
  return DoctorOutputSchema.parse(result);
}
