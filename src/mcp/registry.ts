...
export const TOOL_DEFINITIONS: ToolDefinition[] = [
  // environment
  tool("mastercam_status", "Report live Mastercam connection status and adapter identity", inspection.mastercamStatusSchema, readAnnotations, inspection.mastercamStatusOutputSchema),
  tool("mastercam_capabilities", "Report which capabilities the active backend genuinely supports", inspection.mastercamCapabilitiesSchema, readAnnotations, inspection.mastercamCapabilitiesOutputSchema),
  ...
  // document inspection
  tool("get_active_part", "Inspect the active Mastercam part document", inspection.getActivePartSchema, readAnnotations, inspection.getActivePartOutputSchema),
  ...
  // operations
  tool("list_operations", "List operations with explicit quantities and pagination", inspection.listOperationsSchema, readAnnotations, inspection.listOperationsOutputSchema),
  tool("get_operation", "Retrieve one operation by exact id; no fallback targeting", inspection.getOperationSchema, readAnnotations, inspection.getOperationOutputSchema),
  tool("get_operation_parameters", "Retrieve one operation's parameters by exact id", inspection.getOperationParametersSchema, readAnnotations, inspection.getOperationParametersOutputSchema),
  ...
  tool("verify_change", "Reread an operation and verify an expected feed or speed value (requires exact operationId and quantity)", mutations.VerifyChangeSchema, readAnnotations, mutations.verifyChangeOutputSchema),
  ...
  tool("validate_machine_profile", "Validate an operation against a declared machine profile", inspection.ValidateMachineProfileSchema, readAnnotations, machine.ValidateMachineProfileOutputSchema),

  // controlled mutation workflow
  tool("preview_operation_parameters", "Preview a feed/spindle change and receive a single-use approval token", mutations.PreviewOperationParametersSchema, previewAnnotations, mutations.previewOperationParametersOutputSchema),
  tool("apply_operation_parameter_preview", "Apply a previewed change using its approval token; refuses stale state", mutations.ApplyOperationParameterPreviewSchema, mutationAnnotations, mutations.applyOperationParameterPreviewOutputSchema),
  tool("rollback_change", "Roll back an applied transaction using its server-held receipt", mutations.RollbackChangeSchema, mutationAnnotations, mutations.rollbackChangeOutputSchema),
