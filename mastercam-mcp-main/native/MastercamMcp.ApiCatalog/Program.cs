...
foreach (var file in wanted.Select(name => Path.Combine(root, name)).Where(File.Exists))
{
    try
    {
        ...
        var resolverPaths = Directory.EnumerateFiles(root, "*.dll")
            .Concat(Directory.EnumerateFiles(runtimeDirectory, "*.dll"))
            .Concat(Directory.Exists(frameworkReferenceDirectory) ? Directory.EnumerateFiles(frameworkReferenceDirectory, "*.dll", SearchOption.AllDirectories) : Array.Empty<string>())
    ...
    catch (Exception ex) { assemblies.Add(new { file = Path.GetFileName(file), error = ex.GetType().Name, message = ex.Message }); }
}
...
foreach (var loadContext in loadContexts) loadContext.Dispose();
